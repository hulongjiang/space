...
import (
	...
	helloworld "space/helloworld/proto"
)
...
func (e *Helloworld) Stream(ctx context.Context, req *helloworld.Request, stream helloworld.Response) error {
	log.Infof("Received Helloworld.Stream request with count: %d", req.GetName())

	for i := 0; i < int(req.Count); i++ {
		log.Infof("Responding: %d", i)
		if err := stream.Send(&helloworld.Response{
			Count: int64(i),
		}); err != nil {
			return err
		}
